% Charger l'image
img = imread('30-60/4743.png'); % Remplacez par le chemin de votre image
grayImg = rgb2gray(img); % Convertir en niveaux de gris

% Initialiser les détecteurs Viola-Jones pour chaque composant du visage
...
for i = 1:size(faceBBox, 1)
    % Définir la région supérieure du visage pour les yeux
...
for i = 1:size(faceBBox, 1)
    % Définir la région centrale pour le nez
...
for i = 1:size(faceBBox, 1)
    % Définir la région inférieure du visage pour la bouche
    lowerFace = [faceBBox(i, 1), faceBBox(i, 2) + faceBBox(i, 4) * 0.5, ...
                 faceBBox(i, 3), faceBBox(i, 4) * 0.5];
...
imshow(annotatedImg);
title('Détection du visage, des yeux, du nez et de la bouche');
